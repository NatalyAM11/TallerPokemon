...
public class Register {
	...
	public Register(PApplet app) {
		...
		for (int i = 0; i < inputs.length; i++) {
			cp5.addTextfield(inputs[i]).setPosition((210), 41 + (i * 60)).
			setSize(300, 40).setAutoClear(true)
	...
	public void botonPresionado() {
		...
		if (app.mouseX > 257 && app.mouseX < 465 && app.mouseY > 322 && app.mouseY < 368) {
			...
			if (email.contains("@")) {
				...
				correo = false;
				System.out.println("Insertar @ en el correo");
			}
			...
			if (nombre && contrasena && contrasena && correo) {
				...
				System.out.println("Insertar @ en el correo" + correo);
