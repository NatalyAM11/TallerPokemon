...
public class Logic {
	...
	PImage bOrdenarF, bOrdenarFN, fondo, arbol, inventarioIcono, fondoRosado, pokedexVolver, inventarioVolver,
	...
	public Logic(PApplet app) {
		...
		// Recorremos la matriz para que el personaje pueda moverse por el mapa, lo
		// igualamos con las variables globales
	...
	public void pintar() {
		...
		case 2:
			...
			// invisibilidad CAMBIAR ESTA CONDICIÓN PARA VOLVER A MOVER 
			if(volver == false) {
	...
	public void mouse() {
		...
		case 2:
			...
			if (invisibles == false && quitarAnuncio == true) {
	...
	public void recorrerMatriz() {
		...
		if (mover == true) {
			if (app.keyCode == app.LEFT) {
				if (posXM > 0) {
					if (posXM - 1 < 50 && map[posYM][posXM - 1] == 0 || map[posYM][posXM - 1] == 3
							|| map[posYM][posXM - 1] == 4 || map[posYM][posXM - 1] == 5 || map[posYM][posXM - 1] == 7
							|| map[posYM][posXM - 1] == 6) {
						x -= 70;
						JM = 1;
					}
			...
			if (app.keyCode == app.UP) {
				...
				if (posYM > 0) {
					if (posYM - 1 < 11 && map[posYM - 1][posXM] == 0 || map[posYM - 1][posXM] == 3
							|| map[posYM - 1][posXM] == 4 || map[posYM - 1][posXM] == 5 || map[posYM - 1][posXM] == 7
							|| map[posYM - 1][posXM] == 6) {
						y -= 70;
						JM = 3;
					}
